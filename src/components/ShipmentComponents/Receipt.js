...
const Receipt = ({
  ...
  const flatFee = 200;
  const countryMultiplier = selectedCountry
  ...
  return (
    <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-md w-full h-48 mb-4 receiptCard">
      ...
      {selectedCountry && weightOption && (
        <>
          <p className="receiptText">Destination: {selectedCountry.name}</p>
          <p className="receiptText">
            Weight Option: {weightOption} ({weight}KG)
          </p>
          <p className="receiptText">Box Color: {boxColor}</p>
          <p className="receiptText">Country Multiplier: {countryMultiplier}</p>
          <hr className="my-2" />
          <p className="receiptText">
            Total cost:
            <br />
